...
Begin VB.Form frmAddWHVoucher 
   ...
   Begin Threed.SSFrame SSFrame1 
      ...
      Begin prjMtpTax.uctlTextLookup uctlCompanyLookup 
         ...
         _ExtentY        =   714
      End
      Begin prjMtpTax.uctlDate uctlDocumentDate 
         Height          =   405
         Left            =   7080
         TabIndex        =   3
         Top             =   1830
      ...
      End
      Begin prjMtpTax.uctlTextLookup uctlAccountLookup 
         Height          =   405
         Left            =   1860
         TabIndex        =   1
         Top             =   1380
...
Dim BD As CGLJnl
   ...
   While Not Rs.EOF
      ...
      If Not IsIn(m_TempCol2, BD.VOUCHER) Then
         Call TempCol.Add(BD)
      End If
...
Dim itemcount As Long
   ...
   If Flag Then
      ...
      m_GlJnl.FROM_VOUCHER_DATE = uctlFromDate.ShowDate
      m_GlJnl.TO_VOUCHER_DATE = uctlDocumentDate.ShowDate
      m_GlJnl.AccNumSet = GenerateAccountSet
      If Not glbDaily.QueryGlJnl(m_GlJnl, m_Rs, itemcount, IsOK, glbErrorLog) Then
...
Dim L As CTaxDocItem
Dim TempGnl As CGLJnl

   If ID > 0 Then
      Set TempGnl = TempCol1(ID)

      Set L = New CTaxDocItem
      L.PAY_DATE = TempGnl.VOUDAT
      L.REF_NO = TempGnl.VOUCHER
      
      Set frmAddEditTaxDocItemEx.TaxDocItem = L
...
Dim LastDate As Date
   ...
   If Not m_HasActivate Then
      ...
      Call GetFirstLastDate(DocumentDate, FirstDate, LastDate)
      uctlFromDate.ShowDate = FirstDate
      uctlDocumentDate.ShowDate = LastDate
      
      Call EnableForm(Me, False)
      Call LoadScComp(uctlCompanyLookup.MyCombo, m_Companies)
      Set uctlCompanyLookup.MyCollection = m_Companies
      
      Call LoadMaster(uctlAccountLookup.MyCombo, m_Accounts, MASTER_ACCOUNT)
...
Dim RealIndex As Long
   ...
   Values(1) = CR.VOUCHER
   Values(2) = RealIndex
   Values(3) = DateToStringExtEx2(CR.VOUDAT)
   Values(4) = CR.VOUCHER
   Exit Sub
...
Dim RealIndex As Long
   ...
   Values(1) = CR.REF_NO
   Values(2) = RealIndex
   Values(3) = DateToStringExtEx2(CR.PAY_DATE)
   Values(4) = CR.REF_NO
   Exit Sub
